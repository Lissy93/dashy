...
The [dashboard-icons](https://github.com/homarr-labs/dashboard-icons) repo by [@walkxcode](https://github.com/walkxcode) provides a comprehensive collection of 360+ high-quality SVG, WebP & PNG icons for commonly self-hosted services. Dashy natively supports these icons, and you can use them just by specifying the icon name (without extension) preceded by `hl-`. See [here](https://github.com/homarr-labs/dashboard-icons/tree/main/webp) for a full list of all available icons. Note that these are fetched and cached straight from GitHub, so if you require offline access, the [Local Icons](#local-icons) method may be a better option for you.
